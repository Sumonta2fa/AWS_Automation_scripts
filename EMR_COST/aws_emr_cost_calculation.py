...
def fetch_inst(client):
    id = []
    response = client.list_clusters(
        CreatedAfter=datetime(2018, 6, 15),
        CreatedBefore=datetime(2018, 6, 17),
        ClusterStates=[
            'TERMINATED'
        ],
    )
...
if __name__ == '__main__':
    ...
    my_dict = yaml.load(open('D:\OneDrive\mw\Project\PYTHON\AWS\EMR_COST\cost.yml'))
    client = boto3.client('emr')
    # instance = fetch_inst(client)
    # instance.append('j-2KTWWVDYMQK8O')
    instance = ['j-2KTWWVDYMQK8O', 'j-2KCEZGZTR7JLE', 'j-2HF6R4T2DS4AH']
